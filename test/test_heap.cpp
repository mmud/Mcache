...
static void add(Container& c, uint64_t val) {
    ...
    Heap::heap_update(c.heap.data(), c.heap.size() - 1, c.heap.size());
}
...
static void del(Container& c, uint64_t val) {
    ...
    if (d->heap_idx < c.heap.size()) {
        Heap::heap_update(c.heap.data(), d->heap_idx, c.heap.size());
    }
    delete d;
    c.map.erase(it);
}
...
static void verify(Container& c) {
    ...
    for (size_t i = 0; i < c.heap.size(); ++i) {
        size_t l = Heap::heap_left(i);
        size_t r = Heap::heap_right(i);
